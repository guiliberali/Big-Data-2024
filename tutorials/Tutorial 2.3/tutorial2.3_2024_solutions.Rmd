...
dfYahoo_summary <- dfYahoo_for_sim %>%
  group_by(arm) %>%
...
# get UCB reward
ucb_reward <- dfYahoo_summary$avg_reward + c*sqrt(log(t)/dfYahoo_summary$n_pulls)
...
# TODO: fill in the correct parameter in the EpsilonGreedyPolicy function below to define the policy
eps_greedy <- EpsilonGreedyPolicy$new(epsilon = 0.1)

# TODO: Add the policy and the bandit below to define the agent
eps_greedy_agent <- Agent$new(eps_greedy, # Add the policy
                              bandit_yahoo) # Add the bandit
...
# select relevant columns of the simulation
